...
typedef struct Interp {
    ...
    udword    time; ///< Last time updated in milliseconds, for cleaning up the list
} Interp;

#define InterpMax     32768
#define InterpTimeout 150
static udword interpCount; // Current count
static udword interpPeak;  // Highest count so far
static Interp interp[ InterpMax ];



/// @todo Can be done in constant time just by using a stack
Interp* interpFind( SpaceObj* obj ) {
    const udword limit = min( InterpMax, interpPeak + 1 );

    for (udword i=0; i<limit; i++)
        if (interp[i].obj == obj)
            return &interp[i];
    return NULL;
}
...
void interpCreate( Interp* interp, SpaceObj* obj ) {
    interp->obj  = obj;
    interp->last = obj->posinfo.position;
    interp->time = SDL_GetTicks();

    if (obj->objtype == OBJ_ShipType) {
        interp->last = ((SpaceObjRotImp *)obj)->collInfo.collPosition;
    }
...
void interpDestroy( Interp* interp ) {
    interpCount--;
...
void interpClean(void) {
    const udword now = SDL_GetTicks();

    for (sdword i=0; i<interpPeak; i++)
        if (now - interp[i].time > InterpTimeout)
            interpDestroy( &interp[i] );
}



void spaceobjInterpolatePosition( SpaceObj* obj ) {
    Interp* interp = interpGet( obj );

    const vector a = interp->last;
    vector b = obj->posinfo.position;

    if (obj->objtype == OBJ_ShipType) {
        b = ((SpaceObjRotImp *)obj)->collInfo.collPosition;
    }

    const real32 f = rintGetValue();
...
void spaceobjRestorePosition( SpaceObj* obj ) {
    Interp* interp = interpGet( obj );

    if (obj->objtype == OBJ_ShipType) {
       ((SpaceObjRotImp *)obj)->collInfo.collPosition = interp->last;
    } else {
        obj->posinfo.position = interp->last;
    }
}
