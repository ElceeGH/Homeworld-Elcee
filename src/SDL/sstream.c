...
static sdword isoundstreamreadheader(STREAM *pstream)
{
    ...
    {
        PATCH *ppatch        = (PATCH *)pqueue->offset;
...
static sdword isoundstreamreadblock(STREAMQUEUE *pqueue, void *buffer, smemsize position, sdword size)
{
    ...
    if (pqueue->flags & SOUND_FLAGS_QUEUESTREAM)
    {
        ...
        fileSeek(pqueue->fhandle, position, FS_Start);
...
//		memset((sbyte *)buffer, 0, size);
    ...
    if (ret != size) {
        dbgFatalf( DBG_Loc, "isoundstreamreadblock(): Requested read with size=%i failed, got %i instead.", size, ret );
        return SOUND_ERR;
    }
...
static sdword streamerBufferSize( sdword bitrate ) {
    const sdword bufferSizeMax = SOUND_STREAM_BUFFER_SIZE;
    const sdword chunkSize     = roundDownToMultiple( bitrate, 8 );
    return roundDownToMultiple( bufferSizeMax, chunkSize );
}



static void streamerFree( void ) {
    ...
    for (sdword i=0; i <numstreams; i++) {
        ...
        memset( streams[i].buffer, 0x00, SOUND_STREAM_BUFFER_SIZE );

        speechchannels[i].currentpos = (sbyte*)streams[i].buffer;
        speechchannels[i].freqdata   = (sbyte*)streams[i].buffer;
    }
...
static sbyte* streamerNextBuffer( const STREAM* const pstream ) {
    return pstream->buffer + (pstream->blocksize * pstream->writeblock);
}



static void streamerStartStream( STREAM* const pstream, CHANNEL* const pchan, STREAMQUEUE* const pqueue ) {
    ...
    pstream->buffersize = streamerBufferSize( pchan->bitrate );
    pstream->blocksize  = pstream->buffersize / 2;
    pchan->endpos       = (sbyte *)(pstream->buffer + pstream->buffersize);
...
static void streamerWriteStream( STREAM* const pstream, CHANNEL* const pchan, STREAMQUEUE* pqueue ) {
    if (pstream->blockstatus[pstream->writeblock] != 0) {
        return;
    }
                
    if (pstream->dataleft == 0) { /* HMMM, WE'RE OUT OF DATA HERE */
    ...
    if (pstream->dataleft >= pstream->blocksize) {
        ...
        pstream->lastpos += isoundstreamreadblock(pqueue, streamerNextBuffer(pstream), pstream->lastpos, pstream->dataleft);
        ...
        const sdword readsize  = pstream->blocksize - pstream->dataleft;
        Uint8* const bufferpos = streamerNextBuffer(pstream) + pstream->dataleft;
        pstream->dataleft = 0;
        ...
        if (pqueue->fhandle != SOUND_ERR) {
            ...
            if (pstream->dataleft >= readsize) { /* read a block of data */
                pstream->lastpos  += isoundstreamreadblock(pqueue, bufferpos, pstream->lastpos, readsize);
                pstream->dataleft -= readsize;
            } else {
                const sdword read = isoundstreamreadblock(pqueue, bufferpos, pstream->lastpos, pstream->dataleft);
                const sdword next = (pstream->writeindex + 1) % SOUND_MAX_STREAM_QUEUE;
                if (pstream->queue[next].fhandle != SOUND_ERR) {
                    memset(bufferpos + pstream->dataleft, 0, readsize - pstream->dataleft);
                } else {
                    pstream->writepos = bufferpos + pstream->dataleft;
                }
    
                pstream->lastpos += read;
                pstream->dataleft = 0;
            }
        } else {
